...
for embedding in "${word_embeds[@]}"; do
    for seed in "${seeds[@]}"; do
        for freeze in "${freezes[@]}"; do
            if [ "$freeze" = true ]; then
                python -m src.droptc.train_classifier --embedding "$embedding" --scenario dronelog --seed "$seed" --feature_col sentence --n_epochs 20 --freeze_embedding 
            else
                python -m src.droptc.train_classifier --embedding "$embedding" --scenario dronelog --seed "$seed" --feature_col sentence --n_epochs 20
            fi
        done
    done
done
